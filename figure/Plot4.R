...
with(dataSubS,plot(strptime(paste(Date,Time,sep=" "),"%Y-%m-%d %H:%M:%S"),Voltage,type='l',ylab="Voltage",xlab="datetime"))
with(dataSubS,{
...
with(dataSubS,plot(strptime(paste(Date,Time,sep=" "),"%Y-%m-%d %H:%M:%S"),Global_reactive_power,type='l',xlab="datetime"))
dev.off()